...
class LoadImages:
    ...
    def __next__(self):
        '''
            ...
            img = treated image respect to the original frame
            img0 = Frame or image
            imgf = Original image - NA HERE as is equal to img0
        ...
        return path, img, img0,'', self.cap, s, self.total_count
...
class LoadROSBAG:
    ...
    def __init__(self, sources, topic, fps=6, img_size=1024, stride=32, auto=True, height=1080, width=1920):
        ...
        self.height = height
        self.width = width
        self.bag_file = rosbag.Bag(sources, "r")
        self.frames = self.bag_file.get_message_count(self.topic)
        self.count = 0

        self.threads = Thread(target=self.update, args=(), daemon=True)
        self.threads.start()
        self.img0 = [None]
        self.imgs = [None]
    ...
    def update(self):
        ...
        '''
        self.stream = self.bag_file.read_messages(topics = [self.topic])
        delta_height = (self.height - self.img_size[0])/2
        delta_width = (self.width - self.img_size[0])/2

        for topic_b, msg, t in self.stream:
            self.img0 = CvBridge().compressed_imgmsg_to_cv2(msg)
    ...
    def __next__(self):
        '''
            ...
            path = Path to the file
            img0 = Raw frame without any treatment
            imgT = Treated frame to adjust to the specific model input dimentions
            imgRP = Resized and Padded image using YOLO
            self.cap = object that hold the media (video or none if is an image)
            s = String with information
            self.total_count = count of images + videos frames).
        ...
        ## Convert
        imgRP = imgRP.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        imgRP = np.ascontiguousarray(imgRP)

        s = f'image {self.count}/{self.frames}'
...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        return img_path, img0, img0, img, self.cap, s, self.frame_count
...
class LoadStreams:
    ...
    def __next__(self):
        ...
        # Letterbox
        img0 = self.imgs.copy()
        imgRP = [letterbox(x, self.img_size, stride=self.stride, auto=self.rect and self.auto)[0] for x in img0]

        # Stack
        imgRP = np.stack(imgRP, 0)

        # Convert
        #print(imgRP.shape)
        imgRP = imgRP[..., ::-1].transpose((0, 3, 1, 2))  # BGR to RGB, BHWC to BCHW
        imgRP = np.ascontiguousarray(imgRP)

        return self.sources, img0, img0, imgRP, None, '', self.count
